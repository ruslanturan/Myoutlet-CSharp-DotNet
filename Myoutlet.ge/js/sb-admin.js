(function($) {
  ...
  // Smooth scrolling using jQuery easing
    ...
    $(".edtPrdct").on("click", function (e) {
        e.preventDefault();
        if (confirm("გსურთ ამ პროდუქტის შესწორება? თუ ამ გვერდზე გადადიხართ ცვლილებები უნდა შეინახოთ.")) {
            window.location.replace($(this).attr("href"));
        }
    });
	...
	$(".adkind").on('click', function (e) {
		...
		if($("#Name").val() == ""){			            
			alert("გთხოვთ, დაამატოთ ქვეკატეგორიის სახელი.");
            return;
		}
        if ($(".img-list").children().length < 2) {
            alert("გთხოვთ, დაამატოთ ქვეკატეგორიის სურათი.");
            return;
        }
	$('.register').submit();
    });
